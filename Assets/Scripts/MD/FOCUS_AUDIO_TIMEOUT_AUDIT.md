...
|---|---|---|---|---|
| 1 | Visibility listener registration (.jslib + wrapper + Awake) | FIXED (was MISSING) | `Assets/Scripts/JS/JSFunctCalls.cs` (`RegisterVisibilityListener`), `Assets/Plugins/WebGL/CustomJsLib.jslib` (`RegisterVisibilityChangeListener`), `Assets/Scripts/Functionality/UIManager.cs` `Awake()` | Added DllImport+wrapper, `.jslib` listener, and the `Awake()` call. Hosted on `UIManager`/GameObject `UIMananger` (literal reference placement) — **requires a manual Editor step**: wire the 3 new `[SerializeField]` fields (`jsFunctCalls`, `audioController`, `socketController`) on the `UIMananger` GameObject's Inspector before this is live. |
| 2 | `OnFocusChanged` public callback | FIXED (was MISSING) | `Assets/Scripts/Functionality/UIManager.cs` | Added `public void OnFocusChanged(string value)`, same file/GameObject as Check 1. |
| 3 | Audio mute/unmute honors user sound setting (both focus paths call one shared mute-flag method, reentrancy-guarded; invariant + reverse invariant; no Pause/UnPause) | FIXED (was FAIL — used `Pause()`/`UnPause()`) | `Assets/Scripts/Functionality/AudioController.cs` | Replaced `OnApplicationFocus`'s Pause/UnPause with `SetMuteAll(bool)` (per-`AudioSource` capture/restore, reentrancy-guarded via `isForceMuted`). `ToggleMute` (user-toggle path) now routes through a `SetSourceMute` helper so the user's own mute/unmute button always wins immediately, even mid-force-mute. |
| 4 | 60s background timeout (`WaitForSecondsRealtime`) | FIXED (was MISSING) | `Assets/Scripts/APIs/SocketController.cs` | Added `HandleFocusChange`/`FocusTimeoutCheck` (ported from reference), `isExiting`/`isBeingDestroyed` guards wired into `CloseSocket()`/`OnDestroy()`. Called only from `UIManager.OnFocusChanged`, never from `AudioController.OnApplicationFocus`. |
| 5 | `OnError` (session-expired vs generic) | PASS (no change) | `Assets/Scripts/APIs/SocketController.cs:278-295` | Already matched the reference; verified only. |
| 6 | Instant JS-side mute (WebAudio suspend in `.jslib`) | FIXED (was MISSING) | `Assets/Plugins/WebGL/CustomJsLib.jslib` | `setUnityAudioSuspended` helper added, called first inside `sendFocusToUnity` — same edit as Check 1. |
| 7 | Orientation change / responsive scaling (`SwitchDisplay` + continuous match) | FIXED (was PARTIAL FAIL) | `Assets/Scripts/OrientationChange.cs` (GameObject `OC`) | Added `Start()` for initial-orientation-on-boot; replaced the discrete `MatchWidth`/`MatchHeight`/`PortraitMatchWandH` bucket logic with the continuous log-interpolation formula in a new `ApplyMatch(int,int)`; removed the now-dead inspector fields. |
| 8 | WebGL template canvas fit (`resizeCanvas` + listeners, macros intact) | FIXED (was FAIL) | `Assets/WebGLTemplates/custom/index.html` | Replaced the hardcoded-16:9-letterbox `resizeCanvas` with the `visualViewport`-based version; added `orientationchange`/`visualViewport`/scroll-lock listeners; fixed `window.addEventListener('load', resizeCanvas())` → `resizeCanvas` (function reference, was calling immediately). Template macros/`#if` blocks were already intact — left as-is. |
| 9 | No blanket console suppression in WebGL template | FIXED (was FAIL) | `Assets/WebGLTemplates/custom/index.html` | Removed the `console.log/warn/error = function(){}` block entirely. |
| 10 | Balance sync socket event (`balance:sync`) | FIXED (was MISSING) | `Assets/Scripts/APIs/SocketModel.cs` (`BalanceSyncPayload`), `Assets/Scripts/APIs/SocketController.cs` (`HandleBalanceSync`, `OnBalanceSync` delegate), `Assets/Scripts/GameManager.cs` (`HandleBalanceSync`) | Added DTO, socket registration + handler (writes `socketModel.playerData.balance`), and a `GameManager` subscriber that snaps `currentBalance`/`uIManager.playerBalance.text` and re-runs the existing low-balance gate. |
| 11 | No high-frequency/heartbeat debug logging | FIXED (was FAIL) | `Assets/Scripts/APIs/SocketController.cs` (`OnPongReceived`, `PingCheck`) | Removed the per-tick `Debug.Log` calls; `Debug.LogWarning`/`Debug.LogError` failure-path logs left untouched. |

**Also fixed (out-of-scope, found during Check 4 research):** `SocketController.cs` `CloseSocket()` had `#if UNITY_WEBGL && !UNIFonTY_EDITOR` (typo for `!UNITY_EDITOR`), which made the guard always-true under `UNITY_WEBGL` including in the Editor. Corrected to `!UNITY_EDITOR`.
